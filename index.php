        ...
        case "show_discount":
            if (! empty($_SESSION["cart_item"])) {
                if (! empty($_POST["discountCode"])) {
                    //return all the column entries by coupon code from the db
                  
                    
                   $priceByCode = $db_handle->runQuery("SELECT * FROM tbl_discount_coupon WHERE discount_code='" . $_POST["discountCode"] . "'");
                    if (! empty($priceByCode)) {
                        foreach ($priceByCode as $key => $value) {
                            //return all the elements of the result set and pass to the method to calculate the final discount price
                            $couponName = $priceByCode[$key]["discount_code"];
                            $totalQuantity = $_POST["totalQuantity"];
                            $discountPercentage = $priceByCode[$key]["coupon_discount_percentage"];
                            $minAmount = $priceByCode[$key]["min_amount"];
                            $maxAmount = $priceByCode[$key]["max_amount"];
                            $setDiscountPrice = $priceByCode[$key]["price"];
                            $cartTotalAmount = $_POST["totalPrice"];
                            

                        }

                        require_once ("discountcoupons.php");
                        $coupon_handler = new DiscountCoupons();
                        $discountPrice = $coupon_handler->applyDiscount($couponName,$totalQuantity,$discountPercentage,
                        $minAmount,$maxAmount,$setDiscountPrice,$cartTotalAmount);

                        // calculate discount price based on preset conditions
...
<HTML>
<HEAD>
<TITLE>eCommerce Shopping Cart with discount coupons</TITLE>
       ...
       $total_price = 0;
        ...
        ?>
                        ...
                        <td align="right"><input
                            type="hidden" name="totalQuantity"
                            id="totalQuantity"
                            value="<?php echo $total_quantity; ?>"><?php echo $total_quantity; ?></td>
